...
interface Task extends Omit<DBTask, 'id' | 'completed' | 'isFrog' | 'isDeleted' | 'projectId'> {
  id: number;
  completed: boolean;
  projectId: number;
}

// Define a specific type for task priorities used in the UI
...
export function ProjectsView() {
  ...
  const [newProjectDueDate, setNewProjectDueDate] = useState<Date | undefined>()
  ...
  const [newTaskPriority, setNewTaskPriority] = useState<UIPriority>("importantNotUrgent")
  ...
  const mapDBTaskToView = (dbTask: DBTask): Task => {
    return {
      ...dbTask,
      id: dbTask.id!,
      completed: dbTask.completed === 1,
      projectId: dbTask.projectId! as number,
      priority: dbTask.priority as UIPriority,
  ...
  const calculateProjectProgress = (tasks: Task[]): { progress: number; totalTasks: number; completedTasks: number } => {
  ...
  const updateProjectProgressInDB = async (projectId: number, tasks?: Task[]) => {
  ...
  const handleCreateProject = async () => {
    if (newProjectForm.name?.trim()) {
      ...
      try {
        ...
        setNewProjectForm({ name: "", description: "", goalId: undefined });
  ...
  const openEditDialog = (project: Project) => {
    ...
    setEditingProjectDueDate(project.dueDate ? new Date(project.dueDate) : undefined);
    setIsEditDialogOpen(true);
  };

  const handleEditProject = async () => {
    if (!editingProject || !editingProjectForm.name?.trim()) return;
    ...
    const projectDataToUpdate: DBProject = {
      ...
      description: editingProjectForm.description || "",
      goalId: editingProjectForm.goalId,
      dueDate: editingProjectDueDate,
  ...
  const updateProjectStatus = async (id: number, status: Project['status']) => {
                  ...
                  <div className="grid gap-2">
                    ...
                    <Popover>
                      <PopoverTrigger asChild>
                        <Button
                          variant={"outline"}
                          className={cn("w-full justify-start text-left font-normal", !newProjectDueDate && "text-muted-foreground")}
                        >
                          <CalendarIcon className="mr-2 h-4 w-4" />
                          {newProjectDueDate ? format(newProjectDueDate, "PPP") : <span>选择日期（可选）</span>}
                        </Button>
                      </PopoverTrigger>
                      <PopoverContent className="w-auto p-0">
                        <CalendarComponent mode="single" selected={newProjectDueDate} onSelect={setNewProjectDueDate} initialFocus />
                      </PopoverContent>
                    </Popover>
                  </div>
                  <div className="grid gap-2">
                    ...
                    <Select
                      value={newProjectForm.goalId?.toString() ?? ""}
                      onValueChange={(valueString) => {
                        const selectedGoal = allGoals.find(g => g.id?.toString() === valueString);
                        setNewProjectForm({ ...newProjectForm, goalId: selectedGoal?.id });
                      }}
                    >
                  ...
                  <div className="mt-4 pt-4 border-t">
                      ...
                      <div className="flex items-center space-x-2">
                        <Input
                          ...
                          className="flex-grow"
                        />
                        <Select value={newTaskPriority} onValueChange={(value) => setNewTaskPriority(value as UIPriority)}>
                            <SelectTrigger className="w-[180px]">
                                <SelectValue placeholder="选择优先级" />
                            </SelectTrigger>
                            <SelectContent>
                                <SelectItem value="importantUrgent">重要且紧急</SelectItem>
                                <SelectItem value="importantNotUrgent">重要不紧急</SelectItem>
                                <SelectItem value="notImportantUrgent">紧急不重要</SelectItem>
                                <SelectItem value="notImportantNotUrgent">不重要不紧急</SelectItem>
                            </SelectContent>
                        </Select>
                        <Button onClick={addTaskToProject} size="sm">
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="sm:max-w-[600px]">
                ...
                <div className="grid gap-2">
                  ...
                  <Popover>
                    <PopoverTrigger asChild>
                      <Button
                        variant={"outline"}
                        className={cn("w-full justify-start text-left font-normal", !editingProjectDueDate && "text-muted-foreground")}
                      >
                        <CalendarIcon className="mr-2 h-4 w-4" />
                        {editingProjectDueDate ? format(editingProjectDueDate, "PPP") : <span>选择日期（可选）</span>}
                      </Button>
                    </PopoverTrigger>
                    <PopoverContent className="w-auto p-0">
                      <CalendarComponent mode="single" selected={editingProjectDueDate} onSelect={setEditingProjectDueDate} initialFocus />
                    </PopoverContent>
                  </Popover>
                </div>
                <div className="grid gap-2">
                  <Label htmlFor="edit-goal">关联目标</Label>
                  <Select
                    value={editingProjectForm.goalId?.toString() ?? ""}
                    onValueChange={(valueString) => {
                      const selectedGoal = allGoals.find(g => g.id?.toString() === valueString);
                      setEditingProjectForm({ ...editingProjectForm, goalId: selectedGoal?.id });
                    }}
                  >
          ...
          <DialogFooter>
            ...
            <Button onClick={handleEditProject}>保存更改</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
